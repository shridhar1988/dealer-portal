...
const CompanyPlantMapping = () => {
  ...
  const exportToCSV = () => {
    let csvContent = "Company Name,Plant Names\n";
      ...
      const companyName = company ? company.companyName : row.companyId;
      const plantName = plant ? plant.plantName : row.plantId;  
      csvContent += `"${companyName}","${plantName}"\n`;
    });
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      allCompanyPlantList.map((row) => {
        ...
        const plant = allPlantList.find(p => p.id == row.plantId);  
        return {
          "Company Name": company ? company.companyName : row.companyId,
          "Plant Name": plant ? plant.plantName : row.plantId,
        };
      })
    );
  ...
  const exportToPDF = () => {
    ...
    autoTable(doc, {
      ...
      body:  allCompanyPlantList.map((row) => {
        ...
        const plant = allPlantList.find(p => p.id == row.plantId);  
        return [
          company ? company.companyName : row.companyId,
          plant ? plant.plantName : row.plantId
        ];
      }),
    });
              ...
              {isLoader && (
                ...
                <div className="card-header">
                  <h3 className="card-title text-sm"><i className='fas fa-arrow-left mr-2' style={{ cursor: 'pointer' }}
                    onClick={() => navigate('/masters')} /> Create Company & Plant Mapping<span
                ...
                <div className="card-body text-sm ">
                  ...
                  <div className='row'>
                    <div className="form-group col-md-3">
                      <label for="usercompanyNameInput" style={{ color: "#000" }}>Select Comany Name<sup style={{ color: "red" }}>*</sup></label>
                      <select
                    ...
                    <div className="form-group col-md-6">
                      <label style={{ color: "#000" }}>Select Plant Name<sup style={{ color: "red" }}>*</sup></label>
                      {/* <select
                        className="select2 w-100"
                        //multiple="multiple"
                        data-placeholder="Select Plant.."
                        //style={{ width: "100%" }}
                       ...
                       {allCompanyPlantList.length > 0 ? (
                        allCompanyPlantList.map((roleObj, index) => (                         
                          <tr>      
                            ...
                            <td>
                              {
                                allCompanyList.find(c => c.id.toString() === roleObj.companyId.toString())?.companyName || roleObj.companyId
                              }
                            </td>
                            <td>
                              {
                                allPlantList.find(c => c.id.toString() === roleObj.plantId.toString())?.plantName || roleObj.plantId
                              }
                            </td>
                            <td className="text-center text-sm">
                              <button type="button" className="btn btn-outline-warning btn-xs"onClick={() => handleEditRoleDetails(roleObj)}>
