    ...
    public class CompanyMasterController : ControllerBase
    {
        ...
        public async Task<IActionResult> addCompany(CompanyDto companyDtos)
        {
            try
            {
                ...
                Log.DataLog(
                    companyDtos.CreatedBy, $"Company Name {companyDtos.CompanyCode} added with description" +
                    $" {companyDtos.CompanyDescription} by the userid {companyDtos.CreatedBy}", "Company Master");
                return Ok(new { success = "True", message = "Company added successfully", data = res });
            }
            catch (Exception ex)
            {
                Log.Error(
                  companyDtos.CreatedBy, $"While creating the Comapny {companyDtos.CompanyCode} following error occured {ex.Message} " +
                  $"", "Company Master");
                return Ok(new { success = "false", message = ex.Message });
            }
        }
