    ...
    public class PlantMasterController : ControllerBase
    {
        ...
        public async Task<IActionResult> addPlant(PlantDto plantDto)
        {
            try
            {
                ...
                Log.DataLog(
       plantDto.CreatedBy, $"Plant Name {plantDto.PlantCode} added with description" +
       $" {plantDto.PlantDescription} by the userid {plantDto.CreatedBy}", "Plant Master");
                return Ok(new { success = "True", message = "Plant added successfully", data = res });
            }
            catch (Exception ex)
            {
                Log.Error(
       plantDto.CreatedBy, $"While creating the Plant {plantDto.PlantCode} following error occured {ex.Message} " +
       $"", "Plant Master");
