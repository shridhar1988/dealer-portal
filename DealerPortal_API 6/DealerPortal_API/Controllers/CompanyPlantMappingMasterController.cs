    ...
    public class CompanyPlantMappingMasterController : ControllerBase
    {
        ...
        public async Task<IActionResult> addCompanyPlantMapping(CompanyPlantMappingDto companyDtos)
        {
            try
            {
                ...
                Log.DataLog(
     companyDtos.ModifiedBy, $"Company id {companyDtos.CompanyId} mapped to plant id {companyDtos.PlantId}", "Company Plant Mapping");
                return Ok(new { success = "True", message = "Company & Plant added successfully", data = res });
            }
            catch (Exception ex)
            {
                Log.DataLog(
companyDtos.ModifiedBy, $"While mapping Company id {companyDtos.CompanyId}  to plant id {companyDtos.PlantId} following error occured : {ex.Message}", "Company Plant Mapping");
