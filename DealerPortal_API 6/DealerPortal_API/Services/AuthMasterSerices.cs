...
namespace DealerPortal_API.Services
{
    public class AuthMasterSerices
    {
            ...
            using (var transaction = await _dbContext.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    if (userWithRole.ProfilePic != null)
                    {
                        ...
                        string portalAddresslink = _configuration["ProfileURL"];
                    ...
                    User user = new User
                    {
                        ...
                        Company = userWithRole.Company,//here takeing company from UI in Account group
                        Plant = userWithRole.Plant,//here takeing plant from UI in joingdate
                    ...
                    await _dbContext.SaveChangesAsync();
                    Log.DataLog(user.UserID.ToString(), $"user created with the mail {user.Email} ", "User Creation Log");
                    ...
                    Log.DataLog($"{userWithRole.CreatedBy}", $"User Creation credentials has been sent successfully to the email {userWithRole.Email}", "User Creation Email Log");
                ...
                if (user1 == null)
                {
                    ...
                    if (user2 == null)
                    {
                        ...
                        if (user != null)
                        {
                            ...
                            //user.FirstName = userWithRole.FirstName;
                            //user.LastName = userWithRole.LastName;
                            //user.Password = Encrypt(userWithRole.Password, true);
                            //user.ConfirmPassword = Encrypt(userWithRole.Password, true);
                            //user.Company = userWithRole.AccountGroup;
                            //user.Plant = userWithRole.JoiningDate;                            
                            ...
                            UpdateField("ConfirmPassword", user.ConfirmPassword, encryptedPassword, val => user.ConfirmPassword = val);

                            UpdateField("Company", user.Company, userWithRole.Company, val => user.Company = val);
                            UpdateField("Plant", user.Plant, userWithRole.Plant, val => user.Plant = val);
                            ...
                            if (userWithRole.ProfilePic != null)
                            {
                                ...
                                UpdateField("ProfilePath", user.ProfilePath, NewfilePath1, val => user.ProfilePath = val);
                                UpdateField("PicDbPath", user.PicDbPath, NewdbfilePath1, val => user.PicDbPath = val);
                            }
                            ...
                            if (empinfo != null)
                            {
                                ...
                                UpdateField("DateOfBirth", empinfo.DateOfBirth, userWithRole.DateOfBirth, val => empinfo.DateOfBirth = val);
                                ...
                                UpdateField("Department", empinfo.Department, userWithRole.AccountGroup, val => empinfo.Department = val);
                                UpdateField("Mobile", empinfo.Mobile, userWithRole.ContactNumber, val => empinfo.Mobile = val);
                                UpdateField("JoiningDate", empinfo.JoiningDate, userWithRole.JoiningDate, val => empinfo.JoiningDate = val);
                                //UpdateField("ModifiedBy", empinfo.ModifiedBy, userWithRole.ModifiedBy, val => empinfo.ModifiedBy = val);
                                //UpdateField("ModifiedOn", empinfo.ModifiedOn, userWithRole.ModifiedOn, val => empinfo.ModifiedOn = val);
                            ...
                            {
                                Log.DataLog(userWithRole.ModifiedBy.ToString(), $"for the user id {user.UserID} role id {oldUserRole.RoleID} was Removed ", "User Role Log");
                                ...
                                _dbContext.UserRoleMaps.Add(userRole);
                                await _dbContext.SaveChangesAsync();
                                Log.DataLog(userWithRole.ModifiedBy.ToString(), $"for the user id {user.UserID} role id {userWithRole.RoleID} was added ", "User Role Log");
                            ...
                            {
                                Log.DataLog($"{userWithRole.ModifiedBy}", $"User Id {user.UserID} updated fields: {string.Join(", ", updatedFields)} By the userId :{userWithRole.ModifiedBy}", "User Update Log");
                            }
                        }
                    }
                    else
        ...
        public async Task<UserWithRole> DeleteUser(string ClientId, Guid UserID, string? loggedUserId)
        {
            try
            {
                var clientcheck = _dbContext.Clients.FirstOrDefault(e => e.Id == ClientId);
                ...
                if (user != null)
                {
                    ...
                    userResult.Password = user.Password;
                    userResult.RoleID = userRole.RoleID;
                    Log.DataLog(loggedUserId.ToString(), $"User id {UserID} data deleted successfully by the Userid : {loggedUserId}", "Delete User Log");
                }
                return userResult;
            }
            catch (Exception ex)
        ...
        public async Task<RoleWithApp> CreateRole(RoleWithApp roleWithApp)
        {
            try
            {
                ...
                await _dbContext.SaveChangesAsync();
                Log.DataLog(roleWithApp.CreatedBy, $"New Role : {roleWithApp.RoleName} Created by the user {roleWithApp.CreatedBy}","Role Log");
                ...
                foreach (int AppID in roleWithApp.AppIDList)
                {
                    ...
                    Log.DataLog(roleWithApp.CreatedBy, $"For the Role : {roleWithApp.RoleName} App id {AppID} was added by the user {roleWithApp.CreatedBy}", "Role App Map Log");
            ...
            try
            {
                ...
                if (role != null)
                {
                    ...
                    foreach (int appID in needToAddAppList)
                    {
                        ...
                        _dbContext.RoleAppMaps.Add(roleApp);
                        Log.DataLog(roleWithApp.ModifiedBy, $"For the Role : {roleWithApp.RoleName} App id {appID} was added by the user {roleWithApp.ModifiedBy} by removing all existing apps", "Update Role App Map Log");
        ...
        public async Task<RoleWithApp> DeleteRole(Guid roleId)
        {
            try
            {
                ...
                Log.DataLog(roleToDelete.CreatedBy, $"Role name {roleToDelete.RoleName} and its mapped apps deleted", "Delete role log");
        ...
        public List<App> GetAllApps()
        {
            try
            {
                var apps = _dbContext.Apps.OrderBy(x=>x.AppName)
        ...
        public async Task<App> CreateApp(App app)
        {
            try
            {
                ...
                await _dbContext.SaveChangesAsync();
                Log.DataLog(app.CreatedBy, $"New App : {app.AppName}  was added by the user {app.CreatedBy}", "App Log");
        ...
        public async Task<App> UpdateApp(App app)
        {
            try
            {
                ...
                Log.DataLog(app.ModifiedBy, $"Existing App : {appToUpdate.AppName} changed to {app.AppName} was added by the user {app.ModifiedBy}", "Update App Log");
        ...
        public async Task<App> DeleteApp(Appdto app)
        {
            try
            {
                ...
                Log.DataLog(appToDelete.CreatedBy, $"App name {appToDelete.AppName}  deleted", "Delete App log");
        ...
        public async Task<AuthTokenHistory> SendResetLinkToMail(EmailModel emailModel)
        {
            try
            {
                ...
                Log.DataLog(user.UserID.ToString(), $"Password reset link has been sent successfully to the mail {emailModel.EmailAddress}", "Password Reset Link Email Log");
                if (!sendresult)
                {
                    ...
                    if (user != null && user.UserName == decryptedArray[1] && forgotPassword.UserID == user.UserID)
                    {
                        ...
                        if (history != null)
                        {
                            ...
                            else
                            {
                                ...
                                Log.DataLog(forgotPassword.UserID.ToString(), $"Password of the user id {forgotPassword.UserID} has been changed successfully", "Password Change Log");
                                user.LastChangedPassword = lastchangedIndex.ToString();
