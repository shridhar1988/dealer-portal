...
namespace DealerPortal_API.Services
{
    public class EmailService
    {
        ...
        public EmailService(IConfiguration configuration, DealerContext dbContext)
        {
        ...
        public async Task<bool> SendInitializationMail(string UserName, string toEmail, string Password, string url)
        {
            try
            {
                ...
                if (ec != null)
                {
                    ...
                    var ebody = _dbContext.MailBodyConfigurations.FirstOrDefault(k => k.MailType == "User Registration");
                    if (ebody == null)
                    {
                        throw new Exception("Email configuration not found.");
                    }
                    string emailBody = ebody.MailBody.Replace("@Username@", UserName)
                                                     .Replace("@URL@", url)
                                                     .Replace("@CreatedDate@", System.DateTime.Now.ToString())
                                                     .Replace("@Email@", toEmail)
                    .Replace("You can now log in and begin using the portal.","");
                    string emailSubject = ebody.MailSubject;
                    sb.Append(emailBody);
                    ...
                    await client.SendMailAsync(reportEmail);
                    //Log.DataLog($"{ec.UserName}",$"User Creation email has been sent successfully to the email {toEmail}","User Creation Email Log")
            ...
            catch (SmtpFailedRecipientsException ex)
            {
                ...
                WriteLog.WriteToFile($"User/SendMail/SmtpFailedRecipientsException:- {ex.Message}", ex);
                throw new Exception("Failed to send the email credentials");
                return false;
            }
            catch (SmtpException ex)
            {
        ...
        public async Task<bool> SendMail(string code, string UserName, string toEmail, string userID, string siteURL)
        {
            ...
            {
                var ec = _dbContext.EmailConfiguration.FirstOrDefault(k => k.IsActive && !k.IsSSL && k.ID == 1);
                siteURL = _configuration["PassowrdResetURL"];
                if (ec != null)
                {
