...
const Company = () => {
  ...
  const GetRoles = async () => {
    ...
    try {
      const response = await axios.get(`${config.API_URL}CompanyMaster/GetAllCompany`);
      console.log("Get All Company response",response.data.data);
...
debugger;
      const response = isEditMode
        ? await axios.put(`${config.API_URL}CompanyMaster/updateCompany?Id=${appData.appID}`, appData)
        : await axios.post(`${config.API_URL}CompanyMaster/addCompany`, appData);
  ...
  const handleEdit = (app) => {
    ...
    setAppName(app.companyCode);
    setAppRoute(app.companyDescription);
    setIsEditMode(true);
    setEditAppId(app.id);
  ...
  const deleteApp = async (appId) => {
    ...
    try {
      const response = await axios.delete(`${config.API_URL}companyMaster/deleteCompany?id=${roleToDelete}&modifiedBy=${UserId}`,
        {
          data: {
            appID: roleToDelete,
  ...
  const exportToCSV = () => {
    let csvContent = "Company Code,Company Description\n";
    filteredApps.forEach((app) => {
      csvContent += `${app.companyCode},${app.companyDescription}\n`;
    });
    const blob = new Blob([csvContent], { type: "text/csv;charset=utf-8;" });
    saveAs(blob, "CompanyList.csv");
  };

  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      filteredApps.map((app) => ({
        "Company Code": app.companyCode,
        "Company Description": app.companyDescription,
      }))
    );
  ...
  const exportToPDF = () => {
    ...
    autoTable(doc, {
      head: [["Company Code", "Company Description"]],
      body: filteredApps.map((app) => [app.companyCode, app.companyDescription]),
    });
  ...
  const printTable = () => {
    ...
    printWindow.document.write("<html><head><title>Print Companys</title></head><body>");
    printWindow.document.write("<h3>Company List</h3>");
    printWindow.document.write("<table border='1' cellspacing='0' cellpadding='5'><thead><tr><th>Company Code</th><th>Company Description</th></tr></thead><tbody>");
    filteredApps.forEach((app) => {
      printWindow.document.write(`<tr><td>${app.companyCode}</td><td>${app.companyDescription}</td></tr>`);
    });
          ...
          <div className="container-fluid">
            <div className="row mb-2">
              <div className="col-sm-6">
                <h1 className="m-0">Manage Company
                  <span
                    hover-tooltip='In this you can manage the sidebar tabs which are accessed based on the role by giving the Company Code and Company Description. You create a new, edit and delete the Company Info.'
              ...
              <div className="card">
                <div className="card-header">
                  <h3 className="card-title text-sm"><i className='fas fa-arrow-left mr-2' style={{ cursor: 'pointer' }}
                    onClick={() => navigate('/masters')} /> Create New Company
                    <span
                      hover-tooltip='In this you can create a new and edit the Company by giving the Company Code and Company Description.'
                      tooltip-position="bottom"
                    >
                      <i
                  ...
                  <div className='row'>
                    <div className="form-group col-md-3">
                      <label className='labelStyle'>
                        Company Code<sup style={{ color: 'red' }}>*</sup>
                      </label>
                      <input
                        type='text'
                    ...
                    <div className='col-md-3 mb-1'>
                      <label className='labelStyle'>
                        Company Description<sup style={{ color: 'red' }}>*</sup>
                      </label>
                      <input
                        type='text'
                        ...
                        placeholder='Enter Company Descrciption'
        ...
        <div className="container-fluid">
          <div className='row'>
            <div className="col-md-12">
              <div className="card">
                <div className="card-header">
                  <h3 className="card-title text-sm">Company List ( {roles.length} )
                    <span
                      hover-tooltip='In this you can see the list of Company in the table and edit, delete the Company by using the action buttons.'
                      tooltip-position="bottom"
                    >
                      <i
                        className="fas fa-info-circle ml-2"
                  ...
                  <table class="table table-bordered table-sm table-striped">
                    ...
                    <thead>
                      <tr>
                        ...
                        <th style={{ fontWeight: '500', fontSize: 'smaller' }}>Company Code</th>
                        <th style={{ fontWeight: '500', fontSize: 'smaller' }}>Company Decription</th>
              ...
              <div className='modal-content'>
                ...
                <div className='modal-body'>
                  <p className='text-center'>
                    Are you sure you want to delete this Company? Once deleted, it
                    cannot be recovered.
                  </p>
                </div>
